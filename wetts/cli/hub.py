...
def download(url: str, dest: str):
    ...
    with tarfile.open(tar_path) as f:
        for tarinfo in f:
            if "/" not in tarinfo.name or tarinfo.isdir():
                continue
            name = tarinfo.name[tarinfo.name.find('/') + 1:]
            save_path = os.path.join(dest, name)
            print('Extracting to {}'.format(save_path))
...
class Hub(object):
    ...
    def get_model(key: str) -> str:
        if key not in Hub.Assets.keys():
            print('ERROR: Unsupported key {} !!!'.format(key))
            sys.exit(1)
        model = Hub.Assets[key]
        model_dir = os.path.join(Path.home(), ".wetts", key)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
            response = requests.get(
                "https://modelscope.cn/api/v1/datasets/wenet/wetts_pretrained_models/oss/tree"  # noqa
            )
