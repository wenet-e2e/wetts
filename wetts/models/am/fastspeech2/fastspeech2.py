...
class FastSpeech2(nn.Module):
    ...
    Phoneme level pitch and energy predictor are used in Variance Adapter. Pitch and energy 
    are quantized and are converted into corresponding embeddings. For mandarian, explicit 
    prosody control is implemented by inserting prosodic structure labels into input phoneme 
    sequences. Embeddings of prosodic structure labels are removed after encoder.
    ...
    def __init__(self, enc_hidden_dim: int, n_enc_layer: int, n_enc_head: int,
        ...
        Args:
            ...
            n_va_conv_filter (int): Number of convolution filters for each variance 
            predictor in variance adapter.
    ...
    def _get_speaker_embedding(self, enc_output: torch.Tensor,
        ...
        If this is a multi-speaker FastSpeech2 and speaker ID is give, speaker ID
        will be added to enc_output.
        ...
        Raises:
            ValueError: Raised when speaker ID is given to a single-speaker FastSpeech2 or
            no speaker ID is given to a multi-speaker FastSpeech2.
        ...
        if self.speaker_embedding is not None:
            if speaker is None:
                raise ValueError(
                    "Speaker ID is not given when using multi-speaker Fastspeech2."
                )
            else:
    ...
    def forward(
        ...
        This function should not be used for inference. Teacher-forcing is applied to predict
        mel.

        Args:
            x (torch.Tensor): Input phoneme sequences.
            x_length (torch.Tensor): A Tensor of shape (b), each element is the length of x. 
            x_token_type (torch.IntTensor): An IntTensor of shape (b,t'). Assume the output of the
            final FFT block is final_FFT_output and the final output of encoder is enc_output. When
            token_type[i,j] is 0, final_FFT_output[i,j,:] will be remove from enc_output.
            duration_target (torch.Tensor): shape (b,t). Ground truth duration for each phoneme.
            pitch_target (torch.Tensor): shape (b,t). Ground truth pitch for each phoneme.
            energy_target (torch.Tensor): shape (b,t). Ground truth energy for each phoneme.
            speaker (torch.Tensor, optional): shape (b). Speaker ID for each training sample. 
            Defaults to None.

        Returns:
            Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]: predicted mel, 
            predicted pitch, predicted energy, predicted duration in log domain.
        """
        # If x_padding_mask[i,j] is True, x[i,j,:] will be masked in encoder self-attention.
    ...
    def inference(self,
        ...
        Args:
            x (torch.Tensor): Input phoneme sequences.
            x_length (torch.Tensor): A Tensor of shape (b), each element is the length of x. 
            x_token_type (torch.IntTensor): shape (b,t'). Assume the output of the final FFT
            block is final_FFT_output and the final output of encoder is enc_output. When
            token_type[i,j] is 0, final_FFT_output[i,j,:] will be remove from enc_output.
            p_control (float): shape (b,t). Ground truth duration for each phoneme.
            e_control (float): shape (b,t). Ground truth pitch for each phoneme.
            d_control (float): shape (b,t). Ground truth energy for each phoneme.
            speaker (torch.Tensor, optional): shape (b). Speaker ID for each training sample. 
            Defaults to None.
        Returns:
        ...
        # If mel_mask[i,j] is True, x[i,j,:] will be masked in decoder self-attention.
