...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        # if kwargs has spk_emb_dim, then add a linear layer to project
        # spk_emb_dim to hidden_channels
        self.cond_layer_idx = self.n_layers
        if "gin_channels" in kwargs:
            self.gin_channels = kwargs["gin_channels"]
            if self.gin_channels != 0:
                self.spk_emb_linear = nn.Linear(self.gin_channels,
                                                self.hidden_channels)
                # vits2 says 3rd block, so idx is 2 by default
                self.cond_layer_idx = (kwargs["cond_layer_idx"]
                                       if "cond_layer_idx" in kwargs else 2)
    ...
    def attention(self, query, key, value, mask=None):
        ...
        if mask is not None:
            ...
            if self.block_length is not None:
                ...
                assert t_s == t_t, msg
                block_mask = (
                    torch.ones_like(scores).triu(-self.block_length).tril(
                        self.block_length))
...
class FFT(nn.Module):
    ...
    def __init__(self,
        ...
        if isflow and "gin_channels" in kwargs and kwargs["gin_channels"] > 0:
            cond_layer = torch.nn.Conv1d(kwargs["gin_channels"],
                                         2 * hidden_channels * n_layers, 1)
            self.cond_pre = torch.nn.Conv1d(hidden_channels,
                                            2 * hidden_channels, 1)
